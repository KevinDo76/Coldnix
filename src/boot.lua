...
local updateExeEnv = function()
    for i,v in pairs(_G) do
        if i~="_G" and i~="termGen" and i~="SandBox" and i~="ExecutionEnv" and i~="getExecutionEnvType" and not SandBox[i] then
            SandBox[i] = _G[i]
        end
    end
    SandBox._G = SandBox
...
for i,v in ipairs(systemFiles) do
    ...
    local status=xpcall(function() loadfile(v)() end, recordTraceback or function() end)
    if not status then
        KernelPanic(lastError, currentTraceback)
    end
    if Log then
