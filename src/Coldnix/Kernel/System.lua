...
System.utility.getPrefixWorkingDir = function(driveAddress)
    driveAddress=driveAddress or WORKINGDRIVEADDRESS
    return ((string.sub(driveAddress,1,4) == string.sub(BOOTDRIVEADDRESS,1,4) and "boot") or string.sub(driveAddress,1,4))..":"
end
...
System.utility.sanitizePath = function (path) 
    ...
    if includeDriveChange and validAddress then path="/"..string.sub(driveAddress or "",1,4)..":" end
...
System.utility.resolveDriveLookup = function (rawinput)
    local path = System.utility.resolveFilePath(rawinput)
    if path:find(":") then
        ...
        if #chunks[1]==4 and System.utility.resolveFullDriveAddress(chunks[1]) then
            return System.utility.resolveFullDriveAddress(chunks[1]) or "",processedChunks[2] or "",true,true 
        else
