local module = {}
    ...
    module.func = function (rawText)
        ...
        if args[2]~=nil and args[2]~="" then
            local driveAddress,filePath,validdrive,drivelookup = System.utility.resolveDriveLookup(args[2])
            ...
            if (validdrive or not drivelookup) and drive.exists(filePath) and not drive.isDirectory(filePath) then
                ...
                if not succ then
                    ...
                    print('Executed "'..System.utility.getPrefixWorkingDir(driveAddress)..filePath..'"')
                end
            else
                if validdrive and drive.isDirectory(args[2]) then
                    print('"'..args[2]..'" is a directory')
                else
                    print('Unable to find an executable at "'..System.utility.getPrefixWorkingDir(driveAddress)..filePath..'"')
                end
            end
        else
